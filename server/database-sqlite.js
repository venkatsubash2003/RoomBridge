...
CREATE TABLE IF NOT EXISTS user_roles (
  ...
  created_at TEXT DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY(user_id,role)
);
...
CREATE TABLE IF NOT EXISTS saved_profiles (
  ...
  created_at TEXT DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY(user_id,saved_user_id)
);
...
CREATE TABLE IF NOT EXISTS user_enforcements (
  ...
  reason TEXT NOT NULL, starts_at TEXT DEFAULT CURRENT_TIMESTAMP, expires_at TEXT,
  created_by TEXT NOT NULL REFERENCES users(id), revoked_at TEXT, revoked_by TEXT REFERENCES users(id)
);
CREATE TABLE IF NOT EXISTS moderation_appeals (
  id TEXT PRIMARY KEY, user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  enforcement_id TEXT NOT NULL REFERENCES user_enforcements(id) ON DELETE CASCADE,
  statement TEXT NOT NULL, status TEXT DEFAULT 'pending',
  reviewed_by TEXT REFERENCES users(id), decision_reason TEXT,
...
CREATE TABLE IF NOT EXISTS moderation_actions (
  ...
  reason TEXT NOT NULL, metadata TEXT DEFAULT '{}', created_at TEXT DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS life_state (
  user_id TEXT PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE,
  data TEXT NOT NULL DEFAULT '{}', updated_at TEXT DEFAULT CURRENT_TIMESTAMP
);
CREATE TABLE IF NOT EXISTS audit_events (
  id TEXT PRIMARY KEY, actor_id TEXT REFERENCES users(id), action TEXT NOT NULL,
  target_type TEXT, target_id TEXT, outcome TEXT NOT NULL, ip TEXT, user_agent TEXT,
...
CREATE TABLE IF NOT EXISTS idempotency_keys (
  ...
  route TEXT NOT NULL, status INTEGER NOT NULL, response TEXT NOT NULL,
  expires_at TEXT NOT NULL, created_at TEXT DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY(user_id,key,route)
);
CREATE TABLE IF NOT EXISTS upload_intents (
  id TEXT PRIMARY KEY, user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  purpose TEXT NOT NULL, object_key TEXT UNIQUE NOT NULL, filename TEXT NOT NULL,
  content_type TEXT NOT NULL, byte_size INTEGER NOT NULL, status TEXT DEFAULT 'pending',
  scan_result TEXT, expires_at TEXT NOT NULL, created_at TEXT DEFAULT CURRENT_TIMESTAMP
);
CREATE TABLE IF NOT EXISTS upload_bindings (
  upload_id TEXT PRIMARY KEY REFERENCES upload_intents(id) ON DELETE CASCADE,
  target_type TEXT NOT NULL, target_id TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS jobs (
  id TEXT PRIMARY KEY, kind TEXT NOT NULL, payload TEXT NOT NULL, status TEXT DEFAULT 'queued',
  attempts INTEGER DEFAULT 0, available_at TEXT DEFAULT CURRENT_TIMESTAMP,
  locked_at TEXT, locked_by TEXT, last_error TEXT, created_at TEXT DEFAULT CURRENT_TIMESTAMP,
  completed_at TEXT
);
...
CREATE INDEX IF NOT EXISTS idx_audit_target ON audit_events(target_type,target_id,created_at);
CREATE INDEX IF NOT EXISTS idx_enforcements_active ON user_enforcements(user_id,action,revoked_at,expires_at);
CREATE INDEX IF NOT EXISTS idx_appeals_queue ON moderation_appeals(status,created_at);
...
export const sql = {
  async one(statement, ...params) { return db.prepare(statement).get(...params.map(value=>value===undefined?null:value)); },
  async all(statement, ...params) { return db.prepare(statement).all(...params.map(value=>value===undefined?null:value)); },
  async run(statement, ...params) { return db.prepare(statement).run(...params.map(value=>value===undefined?null:value)); },
  async transaction(fn) {
    db.exec("BEGIN IMMEDIATE");
    const tx = {
      one: async (statement, ...params) => db.prepare(statement).get(...params.map(value=>value===undefined?null:value)),
      all: async (statement, ...params) => db.prepare(statement).all(...params.map(value=>value===undefined?null:value)),
      run: async (statement, ...params) => db.prepare(statement).run(...params.map(value=>value===undefined?null:value))
    };
    try { const result=await fn(tx);db.exec("COMMIT");return result; }
    catch(error){db.exec("ROLLBACK");throw error}
  }
};
export const databaseKind = "sqlite";
